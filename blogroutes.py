...
from db import database, blog, comments
...
class CommentEndpoint(HTTPEndpoint):
    ...
    async def put(self, request: Request):
        ...
        if not comment_id:
            return JSONResponse({"error": "Comment ID path parameter is required"}, status_code=400)
        data = await request.json()
        comment_text = data.get("comment_text")
        if not comment_text:
            return JSONResponse({"error": "Comment text is required"}, status_code=400)
        query = update(comments).where(comments.c.comment_id == comment_id).values(comment_text=comment_text)
        await database.execute(query)
        return JSONResponse({"message": f"Comment with ID {comment_id} updated successfully"})
    ...
    async def delete(self, request: Request):
        ...
        await database.execute(query)
        return JSONResponse({"message": f"Comment with ID {comment_id} deleted successfully"})